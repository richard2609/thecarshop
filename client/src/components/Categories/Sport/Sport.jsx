...
const Sport = ({updateCart, setCart, cart}) => {
    
    const [products, setproducts] = useState([]);
    useEffect(() => {
        try {
            async function httpgetProductsData() {
                const reponse = await fetch("http://localhost:6001/categories/sport");
    ...
    return (
        <div>
            ...
            <div className='container'>
                <div className="row">
                    {
                        sportProducts.slice(0, 4).map((product, index) => {
                            return (
                ...
                <div className="row">
                    {
                        sportProducts.slice(4, 8).map((product, index) => {
                            return (
                                <div key={product.id} className='col-lg-3 col-5'>
                                    <img src={product.imgUrl} className='clip-image img-fluid'></img>
                                    <div className='card'>
